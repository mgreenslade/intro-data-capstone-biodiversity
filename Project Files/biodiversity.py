...
plt.xticks(range(len(protection_counts)), protection_counts.conservation_status)
# Extra work done to make the graph more presentable
plt.ylabel("# of Species", fontsize=15)
# Title changed to be more accurate to graph
plt.title("Number of Endangered Species by Status", fontsize=18)
plt.yscale('log')
ax.set_ylim([1, 1e5])
plt.xlabel("Conservation Status", fontsize=15)
for i, v in enumerate(protection_counts.scientific_name):
    ax.text(i, v * 1.4, str(v), color='blue', fontweight='bold', horizontalalignment='center', fontsize=15)
...
# Unstack performs the same operation as converting the series to a dataframe, pivoting it, and resetting the index.
category_pivot = category_counts.unstack()
...
contingency = [
    [category_pivot.loc['Mammal']['protected'], category_pivot.loc['Mammal']['not_protected']],
...
plt.xticks(range(len(obs_by_park)), obs_by_park.index)
...
2 weeks for Bryce National Park
1 weeks for Yellowstone National Park
